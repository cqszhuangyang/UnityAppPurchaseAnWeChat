...
//是否有购买权限
- (BOOL)canMakePay {
    //UnitySendMessage(mCallBackObjectName.UTF8String, "canMakePay", "NoPermission");
...
- (void)startBuyProduct:(NSString *)productId username:(NSString *)username {
    ...
    if (self.canMakePay) {
        NSLog(@"getProductInfoById: %@", productId);
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    ...
    if (myProduct.count == 0) {
        UnitySendMessage(mCallBackObjectName.UTF8String, "PreBuyProductFailed", "ProductNotExist myProduct.count == 0");
...
- (void)verifyPurchaseWithPaymentTransaction:(SKPaymentTransaction *)transaction restore:(BOOL)restore {
   ...
   // NSString *bodyString = [NSString stringWithFormat:@"%@#%@#%@", transaction.payment.productIdentifier, transaction.transactionIdentifier, receiptString];
    ...
    const char *bodyStr = [receiptString UTF8String];
